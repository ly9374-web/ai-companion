...
export const useLive2DModel = ({
  ...
  useEffect(() => {
    ...
    const applyInitialPosition = (attempt = 0) => {
      if (cancelled) return;
      ...
      if (
        ...
        if (attempt < 30) {
          timer = setTimeout(() => applyInitialPosition(attempt + 1), 100);
        }
        return;
      }

      // Configured shifts use CSS pixels; convert them into Live2D logical
      // coordinates so the visual offset remains stable across DPR values.
      const dprX = canvas.width / canvas.clientWidth;
      const dprY = canvas.height / canvas.clientHeight;
      const shiftedCenterX = canvas.width / 2
      ...
      modelPositionRef.current = initialPosition;
      setPosition(initialPosition);
    };

    // Model replacement starts after 500 ms; wait until the new model matrix
    // is available before applying its configured default position.
    timer = setTimeout(() => applyInitialPosition(), 650);
